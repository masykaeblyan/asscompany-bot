...
client.on('messageDelete', message =>{ // ивент, когда удаляется любое сообщение с сервера https://discord.js.org/#/docs/main/stable/class/Client?scrollTo=e-guildMemberAdd
  ...
  client.channels.cache.get("898957928453779487").send(embed); // айди вашего канала с логами
})
...
async function change() {
    let members = client.guilds.cache.get("898951516998758404").memberCount // сколько людей на сервере + указать айди своего сервера
    client.channels.cache.get("898951516998758400").setName(`На сервере: ${members}`); // свой айди войса
}

var interval = setInterval(function () { change(); }, 10000  ); // время обновления в миллисекундах

client.login('ODk4OTQ2MDA5MzcwMDk1NjQ2.YWrmtg.gCTtUp7uRyVdy5q2g-v6ja7GIKI') // токен вашего бота
